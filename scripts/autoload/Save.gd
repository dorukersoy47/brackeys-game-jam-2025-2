...
func _migrate_save_data() -> void:
								...
								var default_items = ["flour_burst", "oven_mitt", "ice_bath", "tip_jar", "proofing_weight", "till_sweep"]
								var old_items = ["bomb", "shield", "coolant", "harvester", "anchor", "beacon"]
								var items_owned = data["items_owned"] as Dictionary
								
								# Migration: rename old item IDs to new ones
								for i in range(old_items.size()):
										var old_id = old_items[i]
										var new_id = default_items[i]
										if items_owned.has(old_id):
												items_owned[new_id] = items_owned[old_id]
												items_owned.erase(old_id)
