...
func _pick_pattern() -> String:
		...
		return all[rng.randi_range(0, all.size()-2 + int(gs.heat_tier>1))]

func _on_heat(_tier: int) -> void:
		pass

func _on_heat_changed(tier: int) -> void:
		print("DEBUG: PatternController received heat_changed signal, tier: ", tier)
		# Apply speed scaling based on heat tier
		pass
...
func _radial_burst() -> void:
		if not gs or not pool:
				return
		var n := 8 + gs.heat_tier * 12  # More dramatic increase
		var base_speed := 80.0 + gs.heat_tier * 80.0  # Much more dramatic speed increase
		var speed_scale := gs.get_heat_speed_scale()
		var speed := base_speed * speed_scale
		var pos := global_position
		print("DEBUG: _radial_burst - tier: ", gs.heat_tier, " bullets: ", n, " base_speed: ", base_speed, " speed_scale: ", speed_scale, " final_speed: ", speed)
		for i in range(n):
...
func _spiral_stream() -> void:
		if not gs or not pool:
				return
		var pos := global_position
		var rpm := 20.0 + gs.heat_tier * 60.0  # Much more dramatic RPM increase
		var base_speed := 60.0 + gs.heat_tier * 60.0  # Much more dramatic speed increase
		var speed_scale := gs.get_heat_speed_scale()
		var speed := base_speed * speed_scale
		var t := 0.0
		var dur := 2.0
		print("DEBUG: _spiral_stream - tier: ", gs.heat_tier, " rpm: ", rpm, " base_speed: ", base_speed, " speed_scale: ", speed_scale, " final_speed: ", speed)
		while t < dur:
...
func _aimed_volley() -> void:
		...
		var pos := global_position
		var spread := deg_to_rad(15.0 + gs.heat_tier * 10.0)  # Increased from 5 to 10
		var dir := (player.global_position - pos).angle()
		var base_speed := 150.0  # Reduced base speed to make heat scaling more obvious
		var speed_scale := gs.get_heat_speed_scale()
		var speed := base_speed * speed_scale
		print("DEBUG: _aimed_volley - tier: ", gs.heat_tier, " spread: ", spread, " base_speed: ", base_speed, " speed_scale: ", speed_scale, " final_speed: ", speed)
		for angle in [dir-spread, dir, dir+spread]:
				pool.fire(pos, Vector2.RIGHT.rotated(angle) * speed, 4.0)
...
func _wall_sweep() -> void:
		...
		var heat_tier := gs.heat_tier
		var cols := 10 + heat_tier * 2       # grid density
		var rows := 6 + heat_tier            # pulses
		var base_speed := 140.0 + heat_tier * 30.0
		var speed_scale := gs.get_heat_speed_scale()
		var speed := base_speed * speed_scale
		...
		if mode == 0:
				...
				var gap2 := gap if heat_tier < 2 else (gap + rng.randi_range(1, max(1, cols - 1))) % cols
				for step in range(rows):
						if heat_tier >= 1: gap = clamp(gap + rng.randi_range(-1, 1), 0, cols - 1)
						if heat_tier >= 2: gap2 = clamp(gap2 + rng.randi_range(-1, 1), 0, cols - 1)
						for c in range(cols):
				...
				var gap2 := gap if heat_tier < 2 else (gap + rng.randi_range(1, max(1, rows - 1))) % rows
				for step in range(cols):
						if heat_tier >= 1: gap = clamp(gap + rng.randi_range(-1, 1), 0, rows - 1)
						if heat_tier >= 2: gap2 = clamp(gap2 + rng.randi_range(-1, 1), 0, rows - 1)
						for r in range(rows):
...
func _orbit_mines() -> void:
		...
		for i in range(count):
				...
				var speed_scale := gs.get_heat_speed_scale()
				var speed := base_speed * speed_scale
				print("DEBUG: _orbit_mines - tier: ", gs.heat_tier, " count: ", count, " base_speed: ", base_speed, " speed_scale: ", speed_scale, " final_speed: ", speed)
				pool.fire(center, Vector2.RIGHT.rotated(ang) * speed, 3.0)
...
func _flower_pulse(boss := false) -> void:
		...
		var base_speed := 160.0 if boss else 120.0
		var speed_scale := gs.get_heat_speed_scale()
		var speed := base_speed * speed_scale
		var waves := 12 if boss else 6
		print("DEBUG: _flower_pulse - tier: ", gs.heat_tier, " petals: ", petals, " base_speed: ", base_speed, " speed_scale: ", speed_scale, " final_speed: ", speed, " boss: ", boss)
