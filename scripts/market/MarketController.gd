...
func buy_item_midrun(gs: Node, run_inv: Node, def: ItemDef, shrine_price: int) -> bool:
	if int(gs.unbanked) < shrine_price:
		purchase_denied.emit("Not enough unbanked coins.")
		return false
	gs.unbanked = int(gs.unbanked) - shrine_price
	run_inv.add_item(def.id, 1)
	purchase_ok.emit("item_run", def.id)
	return true
